...
b("#wolfnet_keys").append(f).append(c);b("#wolfnet_keyCount").val(a);b("#wolfnet_productKey_"+a).wolfnetValidateProductKey({rootUri:"<?php echo site_url(); ?>?pagename=wolfnet-admin-validate-key"})};b.fn.wolfnetUpdateShortcodeControls=function(a){var c=b(a).find(".keyid").val(),e=function(a){var c=b(a).find("div.wolfnet_loaderImage:first");0==c.length?(c=b("<div/>"),c.append(b('<img src="'+wolfnet_ajax.loaderimg+'" />')),c.addClass("wolfnet_loaderImage"),c.appendTo("#"+b(a).attr("id")+" .wolfnet_prices")):
c.show()};b.ajax({url:wolfnet_ajax.ajaxurl,data:{action:"wolfnet_price_range",keyid:c},dataType:"json",type:"GET",cache:!1,timeout:1E4,statusCode:{404:function(){commFailure()}},beforeSend:function(){e(b(a));b(a).find(".pricerange").each(function(){b(this).prop("disabled",!0)})}}).error(function(a){console.log(a)}).success(function(c){b(a).find(".pricerange").html("");b(a).find(".minprice").append(b("<option />").attr("value","").html("Min. Price"));b(c.min_price.options).each(function(){b(a).find(".minprice").append(b("<option />").attr("value",
this.value).html(this.label))});b(a).find(".maxprice").append(b("<option />").attr("value","").html("Max. Price"));b(c.max_price.options).each(function(){b(a).find(".maxprice").append(b("<option />").attr("value",this.value).html(this.label))})}).always(function(){b(a).find(".pricerange").each(function(){b(this).prop("disabled",!1)});b(".wolfnet_loaderImage").hide()});b.ajax({url:wolfnet_ajax.ajaxurl,data:{action:"wolfnet_saved_searches",keyid:c},dataType:"json",type:"GET",cache:!1,timeout:2500,statusCode:{404:function(){commFailure()}},
success:function(c){c=f(c);b(a).find(".savedsearch").html("");b(a).find(".savedsearch").append(b("<option />").html("-- Saved Search --"));b(c).each(function(){b(a).find(".savedsearch").append(this)})}});b.ajax({url:wolfnet_ajax.ajaxurl,data:{action:"wolfnet_map_enabled",keyid:c},dataType:"json",type:"GET",cache:!1,timeout:2500,statusCode:{404:function(){commFailure()}},success:function(c){!0==c?(b(a).find(".mapDisabled").css("display","none"),b(a).find(".maptype").removeAttr("disabled")):(b(a).find(".mapDisabled").css("display",
"block"),b(a).find(".maptype").attr("disabled","true"))}});var f=function(a){var c=[];b(a).each(function(){c.push(b("<option />").attr("value",this.ID).html(this.post_title))});return c}}}(jQuery);
